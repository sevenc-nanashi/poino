...
function labelParser(log: string) {
  const labelRegexp = new RegExp(
    "^([0-9]+) ([0-9]+) [a-z]+\\^[a-z]+-([a-z]+)\\+[a-z]+=[a-z]+\\/A:(-*[0-9|a-z]+)",
    "gmi"
  )
  ...
  return labels.reverse().map((group) => {
    ...
    if (index === 0) {
      ...
      newAccent = [
        0,
        ...new Array(index).fill(1),
        ...new Array(accents.length - (index + 1)).fill(0),
      ]
    } else {
    ...
    return accents.map((_accent) => {
      ...
      const kana = romaji2kana
        .reduce((kana, x) => {
          const src = x[0]
          const dest = x[1]
          return kana.replace(src, dest)
        }, romaji)
        .replace(new RegExp("[a-z]", "gim"), "")
...
export class OpenJTalk {
  ...
  run(text: string, speed: number) {
    ...
    >((resolve, reject) => {
      ...
      const dicDirPath = path.join(
        this.appDir,
        "./openjtalk/open_jtalk_dic_utf_8-1.11"
      )
      const htsFilePath = path.join(
        this.appDir,
        "./openjtalk/hts_voice_nitech_jp_atr503_m001-1.05/nitech_jp_atr503_m001.htsvoice"
      )
