...
Promise.all([kvs.load(), openjtalk.init(), vocieManager.load()])
  .then(() => app.whenReady())
  .then(async () => {
...
app.on("will-quit", (event) => {
  ...
  Promise.all([removeTmpFiles(), waitSave(kvs)])
    .catch(console.error)
    .finally(() => app.exit())
})
...
ipc.on(
  "synth:run",
  ({ labels, voiceId, speed, volume, pitchMax, pitchMin }, reply) => {
    ...
    synthesizer
      .run(labels, voice, speed, volume, pitchMax, pitchMin, callback)
...
ipc.on("project:open", async (overwriteAlert, reply) => {
  ...
  dialog
    ...
    .then((result): Promise<string | Buffer> => {
      if (result.canceled) return Promise.reject("cancel")
    ...
    .then((result) => {
      if (!projectSavePath) {
        ...
        return ipc
          .send(mainWindow, "ust:decode", ustBinary)
          .then((ust: string) => {
            const notesStr = ust.split(new RegExp("[#[0-9]+]"))

            const tones = [
              27.5, 29.135, 30.868, 32.703, 34.648, 36.708, 38.891, 41.203,
    ...
    .catch((err) => {
      ...
      dialog.showErrorBox("プロジェクトファイルの読み込みに失敗しました", err)
    })
...
ipc.on("project:save", ({ data, newSave }, reply) => {
  ...
  promise
    ...
    .catch((err) => {
      console.error(err)
      if (err === "cancel") return
      dialog.showErrorBox("保存に失敗しました", err)
    })
  ...
  promise
    .then((fileOrDirPath) => {
      if (files.length > 1) {
        return Promise.all(
          files.map((file: any) => {
            return fs.copyFile(file.path, path.join(fileOrDirPath, file.name))
          })
        )
      } else {
        return Promise.all([fs.copyFile(files[0].path, fileOrDirPath)])
      }
    })
    .catch((err) => {
      console.error(err)
      if (err === "cancel") return
      dialog.showErrorBox("書き出しに失敗しました", err)
    })
    .finally(() => reply())
})
      ...
      try {
        ...
        if (
          ...
          Object.entries(obj).every(([licensesClass, licenses]) => {
            ...
            return Object.entries(licenses as object).every(
              ([name, license]) => {
                if (
                  isHash(license) &&
                  "type" in license &&
                  isString(license.type) &&
                  "repo" in license &&
                  isString(license.repo) &&
                  "text" in license &&
                  isString(license.text)
                ) {
                  return true
                } else {
                  return false
...
function getWindowState() {
  ...
  if (isHash(windowState)) {
    ...
    if ("x" in windowState && isNumber(windowState.x)) {
      x = windowState.x
    }

    if ("y" in windowState && isNumber(windowState.y)) {
      y = windowState.y
    }
  }
...
function waitSave(
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      waitSave(target, cancelTimeMs, startTimeMs).then(resolve).catch(reject)
    }, delayTimeMs)
  })
}
...
function removeTmpFiles() {
  return new Promise((resolve, reject) => {
    fs.readdir(tmpDirPath)
      .then((paths) => {
        const tmpFilePaths = paths.filter(
          (_path) => path.extname(_path) === ".tmp"
        )
        return Promise.all(
          tmpFilePaths.map((filePath) => {
            return fs.rm(path.join(tmpDirPath, filePath), { force: true })
          })
        )
      })
      .then(resolve)
...
function projectDiscardConfirm() {
  return new Promise<boolean>((resolve, reject) => {
    ipc
      .send(mainWindow, "project:isSaved", null)
      .then((saved) => {
        if (saved) {
          resolve(true)
          return
